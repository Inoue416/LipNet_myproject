...
class MyDataset(Dataset):
    ...
    def __init__(self, video_path, anno_path, file_list, vid_pad, txt_pad, phase, txt_kind='anno_data_mydic'):
    ...
    def __getitem__(self, idx):
        ...
        path_sep = vid.split('/')
        anno_kind = None
        rand = self.video_data[idx]
        rand = rand.replace('lips', 'rand_label')+'.csv'
        ...
        rand = self._padding(rand, self.vid_pad)
    ...
    def _load_rand(self, p):
        ...
        with open(p, 'r') as csvfile:
            ...
            for row in reader:
                ...
                rand.append(number)
        rand = np.array(rand)
        # 0 - 1に正規化 (もとの画像データが300x300のデータのため)
        rand /= 300.0
