...
model_mode = 'lip_land'
model_name = 'lipnet_land_'+connect_kind+'_'+act

lip_only = False

if lip_only:
    save_prefix = f'_w_{year}_{month}_{day}_t{train_num}v{val_num}_decay{decay}_dp3d{dropout3d_p}_vp{vid_padding}_tp{txt_padding}_batch{batch_size}_ep{max_epoch}_{anno_kind}_{data_len}_{model_mode}_{model_name}_colorMode{color_mode}/{year}_{month}_{day}_t{train_num}v{val_num}_Wdecay{decay}_dp3d{dropout3d_p}_{model_name}_{data_type}'
    log_dir=f"{base_lr}_l_{year}_{month}_{day}_t{train_num}v{val_num}_decay{decay}_dp3d{dropout3d_p}_vp{vid_padding}_tp{txt_padding}_batch{batch_size}_ep{max_epoch}_{anno_kind}_{data_len}_{model_mode}_{model_name}_colorMode{color_mode}"
