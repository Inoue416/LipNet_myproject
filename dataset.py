...
class MyDataset(Dataset):
    ...
    def __init__(self, video_path, anno_path, file_list, vid_pad, txt_pad, phase):
        ...
        self.video_data = []
        # ビデオまでのパスを読み出す
        with open(os.path.join("data", file_list), 'r') as f:
    ...
    def __getitem__(self, idx):
        ...
        if (path_sep[2])[0] == 'r':
            anno_kind = os.path.join('rohan4600-corpus', path_sep[-1])
        else:
            if path_sep[-2] == 'normal':
                number = (path_sep[-1])[-3:]
                anno_kind = os.path.join('ita-corpus', 'normal', 'emotion100_'+number)
            else:
                number = (path_sep[-1])[-3:]
                anno_kind = os.path.join('ita-corpus', 'recitation', 'recitation324_'+number)
        
        vid = self._load_vid(vid) # ビデオデータのフレームをロード
        anno = self._load_anno(os.path.join(self.anno_path, anno_kind+'.txt')) # 正解の文字列データのロード
